...
x.cond.post <- function(y.data, mu, m.nu, V, phi, W, m.0, C.0){
  ...
  for(i in 1:n){
    ...
    m[i+1] = phi*m[i] + A*(y.data[i] - mu - phi*m[i] - m.nu);
    C[i+1] = A*V;
  }
...
for(i in 2:num.samples){
  ...
  mu[i] = mu.cond.post(y.data, x[i-1,], m.nu, V, m.mu, C.mu);
  x[i,] = x.cond.post(y.data, mu[i], m.nu, V, phi[i], W[i], m.0, C.0);
}
